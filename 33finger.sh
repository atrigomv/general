...
variable=`grep 'portid="443"><state state="open"' /root/Escritorio/finger/prueba2/10.10.10.162_nmap.xml`
...
if [ -n "$variable" ]
	...
	echo -e "\e[0;34m[+]  Fuzzing finished\e[0m"
	variable=`grep '<elem key="commonName">' /root/Escritorio/finger/prueba2/10.10.10.162_nmap.xml | cut -d ">" -f 2 | cut -d "<" -f 1 | head -1`
	if [ -n "$variable" ]
		then
	...
	variable=`grep '<elem key="emailAddress">' /root/Escritorio/finger/prueba2/10.10.10.162_nmap.xml | cut -d ">" -f 2 | cut -d "<" -f 1 | cut -d "@" -f 2 | head -1`
	if [ -n "$variable" ]
		then
