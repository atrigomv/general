...
variable=`grep 'portid="445"><state state="open"' /root/Escritorio/finger/prueba2/10.10.10.162_nmap.xml`
if [ -n "$variable" ]
	then
...
variable=`grep 'portid="3389"><state state="open"' /root/Escritorio/finger/prueba2/10.10.10.162_nmap.xml`
if [ -n "$variable" ]
	then
...
variable=`grep 'portid="88"><state state="open"' /root/Escritorio/finger/prueba2/10.10.10.162_nmap.xml`
if [ -n "$variable" ]
	then
...
variable=`grep 'portid="22"><state state="open"' /root/Escritorio/finger/prueba2/10.10.10.162_nmap.xml`
#variable=`grep 'portid="22"><state state="open"' $1/$1_nmap.xml`
if [ -n "$variable" ]
	then
	echo -e "\e[1;35m[+]  SSH service detected!\e[0m"
	variable=`grep 'portid="22"><state state="open"' /root/Escritorio/finger/prueba2/10.10.10.162_nmap.xml | cut -d "<" -f 4 | cut -d '=' -f 4 | cut -d " " -f 1 | cut -d '"' -f 2`
	...
	if [ -n "$variable" ]
		...
		patator ssh_login host=$1 user=root password=FILE0 0=/root/Documentos/pentest/fuzzers/wordlist/top100_passwords.txt -x ignore:mesg='Authentication failed.'
	else
